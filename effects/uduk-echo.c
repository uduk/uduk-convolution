...
double *
echo (double *originalSignal, long originalLen)
{
  float delay = 0.3;
  float alpha = 0.45;
  double D = delay * 44100;

  double *echoSignal = (double *) calloc (originalLen, sizeof (double));

  #pragma omp for schedule(dynamic, CHUNKSIZE)
  for (long i = D; i < originalLen; i++) {
    int x = (int) i - D;
    echoSignal[i] = originalSignal[i] + (alpha * originalSignal[x]);
  }
...
main (int argc, char *argv[])
{
  ...
  double *echoSignal = echo(originalSignal, originalLen);

  writeWav(argv[2], echoSignal, originalLen);
