...
double *
punch (double *originalSignal, long originalLen, float a)
{
 ...
 for (long i = 0; i < originalLen; i++) {
    driveSignal[i] = (0.3 + k) * originalSignal[i] / (0.2 + k * fabs(originalSignal[i]));
  }
...
main (int argc, char *argv[])
{
  ...
  double *punchSignal = punch (originalSignal, originalLen, 0.5);

  writeWav(argv[2], punchSignal, originalLen);
